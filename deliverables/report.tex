...
\subsubsection{Random Effects and Latent Disease Age}
 ...
 Individual trajectories $\gamma_i(t)$ are derived from the average trajectorythrough three types of random effects:
...
\begin{figure}
  ...
  \caption{This figure presents from two points of view (clinical and Riemannian) how the three types of random effects (two temporal and one spatial) enable to modify the population average progression to calibrate the patient observations. 
  \textbf{Clinical}: Two normalised clinical scores (blue and orange) depending on the age of the patients. The scatter represents the real observed values for one patient at different visits. 
...
To model the longitudinal process, we will consider logistic trajectories using the metric $G(p) = 1/[p^2(1-p)^2]$ on the manifold $(0,1)$, which is appropriate for bounded clinical scores. From theren, we can derive the average curve for outcome $k$ at time $t$ as:

\begin{equation}
  \gamma_{0,k}(t) = \left(1 + g_k \times \exp \left(-\frac{(1+g_k)^2}{g_k} (v_{0,k}(t-t_0))\right)\right)^{-1},
\end{equation}
where $t_0$ is the population estimated reference time, $v_{0,k}$ is the speed of the logistic curves at $t_0$, and $p_k = \frac{1}{1+g_k}$ is the value of the modelled outcome at $t_0$. We also get the individual trajectory for patient $i$ and outcome $k$ at time $t$ as:
...
To sum up, the model can be resumed by the following equations for an individual $i$ and an outcome $k$:
...
To perform the optimization procedure, we first compute the likelihood of the model given the observations. For simplification, let $y$ denote the observed data, $z = (z_{\mathrm{fe}}, z_{\mathrm{re}})$ the latent fixed and random effects, $\theta$ the model parameters, and $\Pi$ the known hyperparameters. Parameter estimation is performed by maximizing the following marginal likelihood:
\begin{equation}
...
\begin{figure}[htbp] 
  ...
  \begin{tikzpicture}[
    ...
    \node[box, fill=blue!8]     (ref) {Reference model\\Population parameters $\theta^{*}$};
            \node[box, fill=orange!12,  right=3.0cm of ref] (sim)
              {Synthetic cohort $m$\\$(y_{i,j}^{(m)},\; T_{e_i}^{(m)},\; (B^e_i)^{(m)})$};
    \node[box, fill=green!10,   right=2.8cm of sim] (fit)
          {Fitted model\\ Population parameters $\hat{\theta}^{(m)}$};
    \node[box, fill=red!8,      below=1.8cm of fit] (per)
          {Estimated individual \\ parameters $(\hat{\tau}_i^{(m)},\,\hat{\xi}_i^{(m)}, \dots)$};

    % True individual parameters node
    \node[box, fill=blue!8,    below=1.8cm of sim] (trueip)
          {True sampled individual \\ parameters $(\tau_i^{(m)},\,\xi_i^{(m)}, \dots)$};
    ...
    \draw[arr] (sim.south) -- node[left, font=\footnotesize]{get $(\tau_i, \xi_i, \dots)$} (trueip); 
...
In the scatter plots, one may nevertheless notice a visible concentration of points around estimated values close to zero for some random effects. This pattern could reflect the posterior shrinkage induced by the personalisation step. In finite samples, estimated individual effects are pulled slightly toward the population mean, which is zero for the centred latent effects. This happens especially for individuals with one or few observations, whose personalisation is less informed by data and more influenced by the prior distribution. This phenomenon does not appear when comparing two personalised quantities, such as the reference model and the $m$ fitted model, because both undergo the same restriction. Importantly, this visual effect should not be over-interpreted: despite this mild shrinkage, the agreement metrics remain very good, with ICC values above $0.96$ for all individual effects.
