...
\begin{frame}{Simulation algorithm (1/2): create each follow-up}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.31\textwidth}
      \softbox{softgrey}{
        ...
        $\tau_i\sim\mathcal N(\fitparam{t_0},\fitparam{\sigma_\tau^2})$\\[0.35em]
    ...
    \begin{column}{0.31\textwidth}
      \softbox{softred}{
        ...
        $\designparam{\delta_{f_i}}\sim\mathcal N(\designparam{\bar\delta_f},\designparam{\sigma_{\delta_f}^2})$
      }
    \end{column}
    \begin{column}{0.31\textwidth}
      \softbox{softgreen}{
        \centering\textbf{3. Visit schedule}\\[0.5em]
        draw inter-visit gaps $\designparam{\delta_{v_{i,r}}}$\\
        until the follow-up duration $\designparam{T_{f_i}}$
      }
    \end{column}
  \end{columns}
  ...
  \begin{tikzpicture}[x=1.15cm,y=1cm]
    ...
    \draw[decorate,decoration={brace,mirror,amplitude=5pt}] (0.8,-0.25)--(7.5,-0.25)
      node[midway,below=7pt]{$\designparam{T_{f_i}}$};
  \end{tikzpicture}
