...
The Weibull scale parameter $\tilde{\nu}$ is overestimated by $-35.72\%$ in relative terms. This bias is directly linked to the overestimation of $\sigma_\xi$ through the identifiability constraint implemented in the MCMC-SAEM centering step, which simultaneously adjusts $\log v_0$, $\xi_i$, and $\tilde{\nu}$ by the empirical mean of the current $\xi$ realizations. When $\sigma_\xi$ is overestimated, the empirical spread of $\xi_i$ is inflated, and the resulting centering shift is amplified, propagating a systematic negative shift into $\tilde{\nu} = -\log\nu$. The two biases are therefore not independent: the joint misestimation of $(\sigma_\xi, \tilde{\nu})$ reflects a single underlying confounding between inter-individual progression speed variability and the Weibull time scale.
