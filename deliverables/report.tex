...
L’objectif principal du travail a été d’étendre les capacités de simulation de Leaspy à son modèle conjoint, qui décrit simultanément l’évolution longitudinale des variables cliniques et des données de temps avant événement via une variable latente partagée. Alors que des outils de simulation existaient déjà pour le modèle logistique standard, aucune implémentation intégrée n’était disponible pour le modèle conjoint. Le stage a ainsi consisté à concevoir et implémenter une chaîne de simulation compatible avec ce cadre, incluant la génération des temps d’événement, la prise en compte de la censure, le support d’événements concurrents, ainsi que l’intégration avec les fonctions \texttt{estimate} et \texttt{simulate} de la bibliothèque.
...
Observed scores are further modelled with additive noise of standard deviation $\sigma$, a population parameter estimated jointly with the trajectory parameters.
...
Each synthetic cohort contains $N = 500$ patients, and the experiment is repeated $M = 50$ times. Each fresh model is fitted using the MCMC-SAEM algorithm with $N_{\mathrm{iter}} = 5000$ iterations, and the subsequent personalisation step runs for $N_{\mathrm{perso}} = 1000$ iterations. Naturally, the computational cost of this procedure is substantial, as it requires fitting $M$ separate joint models from scratch. Better results could be obtained by increasing $N$ and $M$, but this would further increase the computational burden. The chosen values of $N$ and $M$ represent a compromise between statistical power and practical feasibility, while still providing a robust assessment of the model's self-consistency.
...
Table~\ref{tab:population_recovery} and Figure~\ref{fig:ree_all} display the performance measures for all population parameters, grouped by quality of recovery. 
...
\hline        
$t_0$                    & 64.92057  & 65.12895  &  0.321  &  0.785  & 0.0072 \\
$\sigma$ (mean)               &  0.10974  &  0.10703  & -2.462  &  2.490  & 0.0038 \\
$\tilde{g}$ (mean)       &  0.83694  &  0.81654  & -2.438  &  4.644  & 0.0409 \\
$\sigma_{\tau}$          & 10.79500  & 10.78870  & -0.058  &  3.632  & 0.0367 \\
$\|\beta\|_F$            &  0.39846  &  0.39425  & -1.057  &  2.539  & 0.0236 \\
$\tilde{v}_0$ (mean)     & -2.79770  & -2.83275  & -1.253  &  2.377  & 0.0202 \\
$\tilde{\rho}$           &  0.80273  &  0.79135  & -1.417  &  6.874  & 0.0689 \\
$\tilde{\nu}$            & -1.55996  & -1.64387  & -5.379  &  6.460  & 0.0343 \\
$\sigma_{\xi}$           &  1.45252  &  1.49225  &  2.735  &  4.308  & 0.0327 \\
$\|\zeta\|$              &  0.16685  &  0.18414  & 10.361  & 41.448  & 0.3673 \\
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/REE_all.png}
  \caption{Relative error of estimation (REE) for all parameters.}
  \label{fig:ree_all}
\end{figure}

The parameters governing the mean longitudinal trajectory and its individual variability are uniformly well identified. The population reference time $t_0$ achieves a relative bias of $0.32\%$ and an RRMSE below $1\%$, with an RSE of $0.007$, the smallest observed across all parameters. This exceptional precision reflects the fact that $t_0$ is the mean of the time-shift distribution and is therefore directly estimated from the centroid of the $N = 500$ individual trajectories. The observation noise standard deviation $\sigma$ is recovered with a small systematic underestimation ($\mathrm{RB} = -2.5\%$), whose magnitude is nearly equal to the RRMSE ($2.5\%$), confirming that the estimator variance is negligible and that the residual error is essentially a stable bias. The log-velocity $\tilde{v}_0$ is recovered with comparable accuracy ($\mathrm{RB} = -1.3\%$, $\mathrm{RRMSE} = 2.4\%$): as the slope of the average logistic trajectory, it is directly identifiable from the rate of change of longitudinal scores across successive visits, and the dense observation schedule ensures that this rate is well determined within each synthetic cohort. The Frobenius norm of the mixing matrix $\|\beta\|_F$, used as an identifiable scalar summary in place of the component-wise values for the reasons discussed above, is also well identified ($\mathrm{RB} = -1.1\%$, $\mathrm{RRMSE} = 2.5\%$), consistently with the fact that the global amplitude of biomarker ordering effects is constrained by the cross-marker correlations visible in the longitudinal data. Finally, the standard deviation of time shifts $\sigma_\tau$ is recovered without systematic bias ($\mathrm{RB} = -0.06\%$), although its RRMSE of $3.6\%$ is somewhat higher than those of the preceding parameters, reflecting the additional Monte Carlo uncertainty introduced by the estimation of the individual random effects from which $\sigma_\tau$ is indirectly inferred.

The initial logit position of the average trajectory $\tilde{g}$ exhibits a downward bias of $-2.4\%$ and an RRMSE of $4.6\%$; the ratio RRMSE/|RB| $\approx 1.9$ reveals run-to-run variability beyond the systematic component, arising from a partial collinearity between $\tilde{g}$, $\tilde{v}_0$, and $t_0$ within the logistic trajectory parameterisation: given data observed over a fixed time window, moderate shifts in $t_0$ can be compensated by correlated adjustments to $\tilde{g}$ or $\tilde{v}_0$ without appreciably changing the likelihood, so finite-sample realisations produce coordinated estimation errors across these three parameters. The log-acceleration standard deviation $\sigma_\xi$ shows a positive bias of $+2.7\%$ ($\mathrm{RRMSE} = 4.3\%$): this slight overestimation of progression rate heterogeneity is to be expected in nonlinear mixed-effects models, where the population variance absorbs residual variability that the individual trajectory model cannot fully account for, in particular when the follow-up duration and data density vary across patients. The Weibull shape parameter $\tilde{\rho}$ is recovered with a small median bias ($\mathrm{RB} = -1.4\%$) but markedly higher run-to-run variability ($\mathrm{RRMSE} = 6.9\%$, $\mathrm{RSE} = 0.069$), as visible from the wide interquartile range in Figure~\ref{fig:ree_all}: shape parameters of Weibull distributions require events to be observed across a broad range of survival times to be well identified, a condition that is only partially satisfied under the censoring pattern of the present study. The Weibull scale parameter $\tilde{\nu}$ displays the largest systematic bias in this group ($\mathrm{RB} = -5.4\%$): a negative relative bias in $\tilde{\nu} = -\log\nu$ implies a systematic overestimation of the scale $\nu$, consistent with the finite-sample behaviour of Weibull estimators under censoring, where censored observations tend to inflate the estimated scale. Additionally, the M-step periodically couples the updates of $-\log\nu$ and $\log v_0$ through the centering of the $\xi$ realizations described in Section~\ref{sec:math-background}, so that finite-sample residuals in this centering propagate as correlated errors across $\tilde{\nu}$ and $\tilde{v}_0$. The relatively moderate RSE of $0.034$ indicates that this bias is stable across repetitions rather than dominated by stochastic variability.

The source-to-survival coupling norm $\|\zeta\|$ stands apart from all other parameters. Its relative bias is already large ($+10.4\%$), but the dominant feature is the extreme estimation variability: $\mathrm{RRMSE} = 41.4\%$ and $\mathrm{RSE} = 0.367$, with the box plot in Figure~\ref{fig:ree_all} spanning a range of approximately $175$ percentage points, the estimated norm fluctuating from values approaching zero to more than twice the true value across repetitions sharing the same experimental design. This failure reveals a fundamental identification difficulty. The parameter $\zeta$ enters the model through the accelerated failure time reparametrisation of Equation~\eqref{eq:nu-rep}, where its influence on the individual survival hazard is mediated by a product with the latent source vector $\mathbf{s}_i$, which is itself subject to estimation uncertainty. With a true norm $\|\zeta\| = 0.167$, the survival shift induced by the biomarker ordering pattern is intrinsically small relative to the dominant individual heterogeneity encoded by $\xi_i$ and $\tau_i$. Furthermore, each patient contributes at most one binary event observation to the estimation of $\zeta$, so its reliable identification requires a strong and consistent covariation between the inferred source patterns and the observed event times across the entire cohort. In the present regime of $N = 500$ patients, a single event type, and non-negligible censoring, this signal is insufficient to yield stable estimates, and the estimation of $\|\zeta\|$ effectively reduces to fitting noise. This finding highlights a practical limitation of the joint model in moderate-sample settings: the survival-source coupling, while theoretically well motivated, may require substantially larger cohorts or higher event rates to be reliably identified.
...
The ICC(3,1) values averaged over the $M = 50$ repetitions are $\mathrm{ICC}(\xi) = 0.981 \pm 0.005$ and $\mathrm{ICC}(\tau) = 0.991 \pm 0.004$, indicating near-perfect pointwise recovery of the log-acceleration and the time shift respectively. As discussed in Section~\ref{sec:metrics}, the ICC(3,1) penalises both lack of linear association and systematic affine offsets, in contrast to Pearson's correlation which is invariant to rescaling; the near-coincidence of the two statistics for both parameters ($r(\xi) = 0.983 \pm 0.005$, $r(\tau) = 0.992 \pm 0.004$) therefore confirms the absence of any meaningful systematic rescaling or shift in the recovered values. The scatter plots of Figure~\ref{fig:param_comparison} provide a direct visual confirmation: both $\xi$ and $\tau$ lie tightly along the identity line across the entire observed range, with the fitted linear regression indistinguishable from perfect agreement at this resolution.

The Bland-Altman analysis of Figure~\ref{fig:bland-altman} completes this characterisation at the individual level. For the log-acceleration $\xi$, the pooled bias across all repetitions and patients is $-0.005$, negligible in absolute terms, and the limits of agreement are $[-0.559,\, +0.549]$, a symmetric band of total width approximately $1.1$ on a scale where the true values span roughly $[-7,\, +7]$ in this dataset. The absence of any dependence of the difference on the mean confirms that no proportional bias operates across the range of individual log-acceleration values. For the time shift $\tau$, the bias is $+0.215$ years, indicating a slight but systematic overestimation of individual disease onset; the limits of agreement are $[-2.518,\, +2.948]$, spanning a total of approximately $5.5$ years. The slight asymmetry of these limits, with the positive bound marginally wider, is consistent with the initialisation constraint built into the joint model personalisation: the time shift is initialised strictly below the observed event time, and the survival component of the posterior likelihood restricts the Gibbs sampler from placing $\tau_i$ beyond the event, which may asymmetrically shift the posterior mean upward relative to the true value. No dependence of the difference on the mean is observed for $\tau$ either, ruling out proportional bias across the full dynamic range of individual time shifts.

Taken together, these results establish a robust and self-consistent individual parameter recovery: both $\xi$ and $\tau$ are recovered with near-perfect agreement between estimated and true values. This conclusion is methodologically reassuring, and of direct relevance for downstream clinical analyses that exploit the personalised parameters to position patients on the disease timeline or to enrich clinical trials with predicted progressors.
...
The validation study reported in this work required fitting $M = 50$ separate joint models from scratch, each involving $5000$ MCMC-SAEM iterations on a cohort of $N = 500$ synthetic patients, followed by a personalisation step of $1000$ iterations per patient. The total computational budget amounted to several hours of continuous execution on institutional computing infrastructure. During the development phase, numerous additional runs were performed for debugging, hyperparameter exploration, and convergence diagnostics, multiplying the effective resource consumption well beyond the final reported experiment.
