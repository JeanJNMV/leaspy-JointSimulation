...
\mention{CentraleSupélec - Mention Sciences des données et de l'information (SDI) \& Filière Métiers de la Recherche (FMR)}
\master{Master Mathématiques Vision Apprentissage (MVA)} % Nom du master
...
\subsection{Challenges in Longitudinal Data Analysis and Existing Approaches}
...
To separate an average disease progression from the individual progression, a mixed-effects model structure is added to the trajectories. Any trajectory $\gamma$ can be defined by the initial point $\gamma(t_0) = p$ and the initial velocity $\dot{\gamma}(t_0) = v_0$. The average disease trajectory is therefore defined by its initial condition $(t_0, p, v_0)$. From there, we can define the individual trajectories $\gamma_i(t)$ using individual time shifts $\tau_i$ and speed $\exp^{\xi_i}$. This leads to the following initial condition $(t_0 + \tau_i, p, v_0 \exp^{\xi_i})$ for each individual $i$. These parameters will be detailed in the following sections.

\subsubsection{Nonlinear Mixed-Effects Models Formalism}

As we mentioned earlier, Leaspy is based on the nonlinear mixed-effects model framework, which decomposes variability in longitudinal observations into two complementary components~\cite{schiratti2017bayesian}:
...
\begin{equation}
y_{i,j,k} = \gamma_{i,k}(t_{i,j,k}) + \epsilon_{i,j,k},
\end{equation} 
where $\epsilon_{i,j,k}$ is an additive noise term, typically assumed to follow a zero-mean Gaussian distribution with outcome-specific variance $\sigma_k^2$. 

\subsubsection{Random Effects and Latent Disease Age}
...
\label{eq:latent-age}
\end{equation} 
which transforms the calendar age $t$ of patient $i$ into a disease-stage age. This formulation provides a biologically interpretable decomposition: $\tau_i$ captures onset heterogeneity and $e^{\xi_i}$ captures progression-rate heterogeneity.

The space shifts $w_{i,k} = (\mathbf{A}\mathbf{s}_i)_k$, obtained via the mixing matrix $\mathbf{A}$ acting on independent sources $\mathbf{s}_i$, capture differences in the sequential ordering of biomarker changes across patients. The columns of $\mathbf{A}$ are constructed from a set of free coefficients $\beta$, jointly estimated with the other population parameters during model fitting.

The effects of all random effects are described in Figure~\ref{fig:random-effects}, which illustrates how the average trajectory is transformed into individual trajectories through time shifts, acceleration factors, and space shifts.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/random_effects.png}
  \caption{This figure presents from two points of view (clinical and Riemannian) how the three types of random effects (two temporal and one spatial) make it possible to modify the population average progression to calibrate the patient observations. 
  \textbf{Clinical}: Two normalised clinical scores (blue and orange) as functions of age of the patients. The scatter represents the real observed values for one patient at different visits. 
  \textbf{Riemannian}: The same two normalised scores are represented but this time depending on each other. The scatter represents the same real observed values as in the clinical version. The black cross on the curve corresponds to what is modelled at the visit ages of the patient \cite{koval2021ad}.}
  \label{fig:random-effects}
\end{figure}

\subsection{Fixed Effects and Population Parameters}
...
\begin{equation}
  \gamma_{0,k}(t) = \left(1 + g_k \times \exp \left(-\frac{(1+g_k)^2}{g_k} (v_{0,k}(t-t_0))\right)\right)^{-1},
\end{equation}
...
\begin{equation}
  \gamma_{i,k}(t) = \left(1 + g_k \times \exp \left(-\frac{(1+g_k)^2}{g_k} (v_{0,k} (\psi_i(t) - t_0) + w_{i,k})\right)\right)^{-1},
\end{equation}

\subsection{Wrap-up and Optimization Procedure}
...
\begin{aligned}
  & \psi_i(t) = e^{\xi_i}(t - \tau_i) + t_0, \\
  & w_{i} = \mathbf{A}\mathbf{s}_i, \\
  & \gamma_{i,k}(t) = \left(1 + g_k \times \exp \left(-\frac{(1+g_k)^2}{g_k} (v_{0,k} (\psi_i(t) - t_0) + w_{i,k})\right)\right)^{-1}, \\
  & y_{i,j,k} = \gamma_{i,k}(t_{i,j,k}) + \epsilon_{i,j,k}, \quad \epsilon_{i,j,k} \sim \mathcal{N}(0, \sigma_k^2).
\end{aligned}
\right.
\end{equation}

To perform the optimization procedure, we first compute the likelihood of the model given the observations. For simplicity, let $y$ denote the observed data, $z = (z_{\mathrm{fe}}, z_{\mathrm{re}})$ the latent fixed and random effects, $\theta$ the model parameters, and $\Pi$ the known hyperparameters. Parameter estimation is performed by maximizing the following marginal likelihood:
\begin{equation}
...
\begin{figure}[htbp]
  ...
  \caption{Disease progression modeling with Leaspy on the built-in Parkinson's disease dataset. \textbf{Left:} Population-average trajectories of the three normalized Movement Disorder Society scores (MDS1, MDS2, and MDS3 in the off-medication state) as functions of chronological age. The colored sigmoid curves represent the fixed-effects component of the model and describe the typical progression of each score in the cohort. \textbf{Right:} Personalized trajectories for one patient. The colored dots are the patient's observed scores at successive visits, while the solid curves are the corresponding subject-specific trajectories reconstructed by Leaspy. The differences between the population and individual curves illustrate how the random effects adapt the timing, progression speed, and relative ordering of the outcomes to the observations of a given patient.}
  \label{fig:parkinson}
\end{figure}
...
\paragraph{Cerebral Autosomal Dominant Arteriopathy with Subcortical Infarcts and Leu-koencephalopathy (CADASIL).}
...
\subsection{Importance in the Leaspy Context}\label{sec:importance-simulation-leaspy}
...
In the original article describing the joint model~\cite{ortholand2026joint}, an algorithm is proposed to simulate synthetic patient cohorts. Before the simulation can be performed, its inputs must be divided into two distinct groups.

\textbf{Parameters learned from the fitted model.} These parameters define the statistical properties of disease progression and are kept fixed during simulation. They include the parameters of the population longitudinal trajectories ($t_0$, $\mathbf{g}$, and $\mathbf{v}_0$), the variability of the individual random effects ($\sigma_\tau$ and $\sigma_\xi$), the observation noise, the Weibull survival parameters ($\nu$ and $\rho$), and, when latent sources are included, the mixing parameters $\mathbf{A}$ and $\boldsymbol{\zeta}$. They are read directly from the fitted joint model and are not specified by the user at simulation time.

\textbf{Parameters specified by the user.} These study parameters determine the size and observation schedule of the synthetic cohort. The user provides the number of patients $N$; the mean and standard deviation of the first-visit offset from disease onset ($\bar{\delta}_f$ and $\sigma_{\delta_f}$); the mean and standard deviation of the per-patient follow-up duration ($\overline{T_f}$ and $\sigma_{T_f}$); and the mean and standard deviation of the inter-visit intervals ($\bar{\delta}_v$ and $\sigma_{\delta_v}$). An additional technical parameter, \texttt{min\_spacing\_between\_visits}, sets the minimum allowed time between two successive visits and prevents numerical artefacts caused by excessively close observations.
...
In the single-event case the event indicator is set to 1 whenever the event time falls within the follow-up window. In the competing-events case ($L > 1$), the event type $l^*$ is drawn at the identified visit time by treating the incremental cause-specific CIF values as unnormalized probabilities:
\begin{equation}
...
\begin{figure}[H] 
  ...
  \begin{tikzpicture}[
    ...
    \path[use as bounding box] (-2.45,-3.25) rectangle (9.65,2.25);
    ...
    \node[box,fill=blue!8] (true) at (0,-2.1) {True sampled effects\\$\tau_i,\xi_i,\ldots$};
    \draw[arr] (ref)--node[left,font=\footnotesize]{simulate (step 1)}(true);
    \draw[arr] (true)--node[midway,below right,font=\footnotesize]{simulate (steps 2-6)}(sim);
    \node[box,fill=green!10] (fit) at (8,0) {Fresh fitted model\\$\hat\theta^{(m)}$};
    \draw[{Stealth[length=6pt]}-{Stealth[length=6pt]},thick] (sim)--node[above,font=\scriptsize]{fit}(fit);
    ...
    \node[box,fill=red!8] (perso) at (8,-2.1) {Personalised effects\\$\hat\tau_i,\hat\xi_i,\ldots$};
    \draw[arr] (fit)--node[right,font=\footnotesize]{personalise}(perso);
    \draw[darr] (ref.north) .. controls +(0,1.75) and +(0,1.75) ..
      node[pos=0.5,above,font=\footnotesize,color=black]{REE, RB, RRMSE, RSE} (fit.north);
    \draw[darr] (true)--node[midway,below=5pt,font=\footnotesize,color=black]{ICC, B-A}(perso);
  \end{tikzpicture}}%
  \caption{Simulation-based evaluation pipeline used to assess the self-consistency of the joint model. Starting from a reference model with known population parameters $\theta^{*}$, the simulation algorithm generates a synthetic cohort $m$ containing longitudinal measurements $y_{i,j}^{(m)}$, event times $T_{e_i}^{(m)}$, and censoring indicators $(B_i^e)^{(m)}$, together with the true individual parameters used for generation. A fresh model is fitted to each cohort to obtain $\hat{\theta}^{(m)}$, and is then personalised to recover the individual parameters $(\hat{\tau}_i^{(m)},\hat{\xi}_i^{(m)},\ldots)$. This simulation, fitting, and personalisation procedure is repeated $M$ times. Population-level recovery is evaluated by comparing $\hat{\theta}^{(m)}$ with $\theta^{*}$ using REE, RB, RRMSE, and RSE, while individual-level recovery is evaluated by comparing true and estimated random effects using the ICC and Bland-Altman analysis.}
...
Table \ref{tab:population_recovery} and Figure \ref{fig:ree_all} summarise the population-parameter recovery results. 
...
\hline
\end{tabular}
\caption{Recovery of the individual effects after fitting and Bayesian personalisation of each of the $M=200$ simulated cohorts, each containing $N=1000$ patients. Within every cohort, the effects estimated after personalisation are compared with the true values used to generate the same patients. The entries report the mean $\pm$ standard deviation of the cohort-specific agreement statistics across the $M$ repetitions. ICC(3,1) is the two-way mixed-effects, single-measure consistency intraclass correlation coefficient and quantifies agreement between true and estimated effects, whereas Pearson's $r$ quantifies their linear association. The temporal effects are the log-acceleration $\xi_i$ and individual reference time $\tau_i$. Because the latent source coordinates are rotationally non-identifiable, spatial recovery is evaluated through the invariant feature-level shifts $w_{i,j}=(\mathbf{A}\mathbf{s}_i)_j$. Indices $j=0,\ldots,3$ denote the bulbar, fine-motor, gross-motor, and total ALSFRS-R outcomes. The quantity $(\boldsymbol{\zeta}^{\top}\mathbf{s}_i)_0$ is the corresponding source-induced survival shift for the single modeled event, death. Values close to one indicate strong recovery.}
\label{tab:metrics}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Estimated versus true individual effects, pooled across all patients and the $M=200$ repetitions. The panels show $\xi_i$, $\tau_i$, the four feature-level spatial shifts $w_{i,j}=(\mathbf{A}\mathbf{s}_i)_j$, and the survival shift $(\boldsymbol{\zeta}^{\top}\mathbf{s}_i)_0$. Each point represents one patient and repetition. The red dashed identity line indicates perfect recovery, while the solid gray line is the linear fit; their close overlap indicates strong agreement.}
...
\begin{figure}[H]
  ...
  \caption{Bland--Altman comparison of the same individual effects across all patients and repetitions. Each panel plots the estimation error against the mean of the true and estimated values. The solid black line is the bias, and the red dashed lines are the limits of agreement, defined as the bias $\pm 1.96$ standard deviations. Biases close to zero and narrow, centered limits indicate accurate personalisation.}
