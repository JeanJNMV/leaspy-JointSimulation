...
\subsubsection{The Original Simulation Algorithm}
...
\subsubsection{The Anchor-Based Simulation Algorithm}
...
\begin{enumerate}
  ...
  \item \textbf{Simulate the event time first}. Before any visit is placed, draw the event time from its Weibull distribution: $T_{e_i} \sim e^{-\xi_i}\, \mathcal{W}(\nu, \rho) + \tau_i$. In case of several competing events, multiple event times are drawn independently and the earliest one is retained as the observed event time.
