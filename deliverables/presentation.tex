...
\newcommand{\validationdiagram}[1]{%
  ...
  \begin{tikzpicture}[
    ...
    \ifnum#1>1
      ...
      \draw[arr] (true)--node[midway,below right,font=\footnotesize]{simulate}(sim);
    \fi
    \ifnum#1>2
      \node[box,fill=green!10] (fit) at (8,0) {Fresh fitted model\\$\hat\theta^{(m)}$};
      \draw[{Stealth[length=6pt]}-{Stealth[length=6pt]},thick] (sim)--(fit);
    ...
    \ifnum#1>4
      ...
      \draw[darr] (true)--node[midway,below=17pt,font=\footnotesize,color=black]{ICC(3,1)}(perso);
    \fi
  \end{tikzpicture}}}
...
\author[Jean-Vincent MARTINI]{Jean-Vincent MARTINI \\[0.6em]
  {\small Supervisor: Dr. Sophie TEZENAS DU MONTCEL}}
...
\begin{frame}{Nonlinear mixed effects: population and individual}
  ...
  \centering
  \includegraphics[width=\textwidth]{figures/short_random_effects.png}
...
\begin{frame}{The joint model: one disease age, two processes}
  ...
  \begin{itemize}
    ...
    \item \textbf{Informative and non-informative dropout:} follow-up can end for a reason related to the patient's
      unobserved health state or for a reason unrelated to the disease.
  \end{itemize}
...
\begin{frame}{Integrating joint simulation inside Leaspy} 
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.31\textwidth}
      \softbox{softgrey}{
        \textbf{1. Standard API}\\[0.2em]
        Extend \texttt{estimate} with longitudinal and event predictions, so joint models use the usual \texttt{simulate} workflow.
      }
    \end{column}
    \pause
    \begin{column}{0.31\textwidth}
      \softbox{softred}{
        \textbf{2. Visit design}\\[0.2em]
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      \softbox{designinput}{
        \textbf{4. Single and competing events}\\[0.2em]
    ...
    \begin{column}{0.48\textwidth}
      \softbox{fitinput}{
        \textbf{5. Rounding-safe censoring}\\[0.2em]
  ...
  \vspace{0.4em}
  \takeaway{One Leaspy workflow now produces bounded trajectories, event types, and coherent censoring.}
\end{frame}
...
\begin{frame}{Three recovery metrics, two inference levels}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \vspace{0.4em}
      \footnotesize
      \textbf{Role in the pipeline.} Compute one ICC per random effect and cohort,
      comparing true sampled with personalised values.

      \vspace{0.5em}
      \softbox{softgrey}{
        \textbf{ICC(3,1): two-way mixed, consistency, single measure}\par\vspace{0.2em}
        Patients are the targets. True and personalised values are the two fixed
        measurement methods ($k=2$).\par\vspace{0.4em}
        {\centering
          $\displaystyle \mathrm{ICC}(3,1)=
          \frac{MS_B-MS_E}{MS_B+(k-1)MS_E}$\par
        }
        \vspace{0.4em}
        $MS_B$: between-patient variation.\\
        $MS_E$: residual true--estimate
        disagreement. \\
...
\begin{frame}{Fixed effects / population parameters: recovery (1/2)}
  \centering
  \small
  ...
  \begin{tabular}{llcc}
    \toprule
    Parameter & Interpretation & RB (\%) [95\% CI] & RRMSE (\%) [95\% CI]\\
    \midrule
    $t_0$ & Population reference time & $0.44\ [0.35,0.53]$ & $0.83\ [0.76,0.90]$\\
    $\sigma_\tau$ & Time-shift SD & $0.08\ [-0.24,0.38]$ & $2.32\ [2.10,2.54]$\\
    $\sigma_\xi$ & Log-acceleration SD & $0.03\ [-0.27,0.37]$ & $2.32\ [2.11,2.56]$\\
    \addlinespace[0.15em]
    $\rho$ & Weibull shape & $0.10\ [-0.61,0.81]$ & $5.01\ [4.50,5.51]$\\
    $\nu$ & Weibull scale & $2.33\ [1.64,3.00]$ & $5.43\ [4.97,5.91]$\\
    \addlinespace[0.15em]
    $\sigma^{(0)}$ & Bulbar observation-noise SD & $-4.60\ [-4.76,-4.45]$ & $4.73\ [4.58,4.89]$\\
    $\sigma^{(1)}$ & Fine-motor observation-noise SD & $-4.16\ [-4.28,-4.04]$ & $4.25\ [4.14,4.38]$\\
    $\sigma^{(2)}$ & Gross-motor observation-noise SD & $-3.08\ [-3.17,-2.97]$ & $3.17\ [3.07,3.27]$\\
    $\sigma^{(3)}$ & Total-score observation-noise SD & $-1.26\ [-1.37,-1.15]$ & $1.47\ [1.37,1.56]$\\
    \bottomrule
  \end{tabular}}
...
\begin{frame}{Fixed effects / population parameters: recovery (2/2)}
  \centering
  \small
  \setlength{\tabcolsep}{4pt}
  ...
  \begin{tabular}{llcc}
    ...
    Parameter & Interpretation & RB (\%) [95\% CI] & RRMSE (\%) [95\% CI]\\
    \midrule
    $g^{(0)}$ & Bulbar trajectory position & $-8.27\ [-9.41,-7.23]$ & $11.41\ [10.60,12.28]$\\
    $g^{(1)}$ & Fine-motor trajectory position & $-8.80\ [-10.03,-7.52]$ & $12.61\ [11.55,13.66]$\\
    $g^{(2)}$ & Gross-motor trajectory position & $-7.60\ [-8.71,-6.52]$ & $11.16\ [10.26,12.06]$\\
    $g^{(3)}$ & Total-score trajectory position & $-5.92\ [-6.68,-5.17]$ & $8.09\ [7.46,8.76]$\\
    \addlinespace[0.15em] 
    $v_0^{(0)}$ & Bulbar trajectory velocity & $6.46\ [5.32,7.65]$ & $10.53\ [9.56,11.53]$\\
    $v_0^{(1)}$ & Fine-motor trajectory velocity & $2.23\ [1.54,2.93]$ & $5.57\ [5.05,6.12]$\\
    $v_0^{(2)}$ & Gross-motor trajectory velocity & $1.12\ [0.52,1.74]$ & $4.52\ [4.12,4.92]$\\
    $v_0^{(3)}$ & Total-score trajectory velocity & $2.05\ [1.37,2.73]$ & $5.25\ [4.75,5.78]$\\
    \bottomrule
  \end{tabular}}

  \vspace{1em}
  \softbox{softred}{
    \textbf{Hardest to recover:} trajectory positions $g$ and bulbar velocity $v_0^{(0)}$, which can compensate for each other.
  }
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.59\textwidth}
      ...
      \begin{tabular}{lc}
        ...
        $\xi_i$ & $0.988\pm0.004$\\
        $\tau_i$ & $0.995\pm0.002$\\
        $w_{i,0}$ & $0.987\pm0.006$\\
        $w_{i,1}$ & $0.989\pm0.003$\\
        $w_{i,2}$ & $0.990\pm0.003$\\
        $w_{i,3}$ & $0.987\pm0.003$\\
...
\begin{frame}{Take-home messages}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Future work:} the same evaluation pipeline will certainly be used during my PhD on "\emph{Explainability and variable selection for multivariate longitudinal data}" as many methods will be developed and tested on synthetic data before being applied to real clinical datasets.
  \end{enumerate}
...
\begin{frame}{Acknowledgements}
  \small
  \softbox{softred}{
    \textbf{Sophie Tezenas du Montcel}\par
    For welcoming me into her research group and for her scientific guidance throughout the internship. 
    \par
    \textbf{Maylis Tran and Sofia Kaisaridi}\par
        For their availability, constructive feedback, and support on the scientific and software aspects of the work.
  }

  \vspace{0.55em} 
  \softbox{softgreen}{
  \textbf{The ARAMIS team}\par
  For the stimulating interdisciplinary environment and the enriching exchanges with researchers, engineers, and fellow interns.}
  \vspace{0.55em}
  \softbox{designinput}{
    \textbf{Arthur Tenenhaus (SDI) and Vincent Mousseau (FMR)} for their academic follow-up;
    the \textbf{Paris Brain Institute, INRIA, and Leaspy contributors} for the institutional,
    computational, and open-source foundations of this work.
  }

  \vfill
  \centering
  \includegraphics[height=0.52cm,keepaspectratio]{logos/logoSorbonne.png}\hspace{0.25cm}
  \includegraphics[height=0.52cm,keepaspectratio]{logos/logoICM.png}\hspace{0.25cm}
  \includegraphics[height=0.52cm,keepaspectratio]{logos/logoCNRS.png}\hspace{0.25cm}
  \includegraphics[height=0.52cm,keepaspectratio]{logos/logoINRIA.png}\hspace{0.25cm}
  \includegraphics[height=0.52cm,keepaspectratio]{logos/logoINSERM.png}\hspace{0.25cm}
  \includegraphics[height=0.52cm,keepaspectratio]{logos/logoAPHP.png}
\end{frame}
...
\begin{frame}{Appendix: detailed individual-effect recovery}
  ...
  \begin{tabular}{lc}
    ...
    $w_{i,2}$ & $0.990\pm0.003$\\
    $w_{i,3}$ & $0.987\pm0.003$\\
    $(\boldsymbol\zeta^\top\mathbf s_i)_0$ & $0.965\pm0.027$\\
    \bottomrule
  \end{tabular}
