...
\todo{INCLURE LES PARTIES CS: RECUL ETHIQUE, GRILLE DE COMPETENCES, REMERCIEMENTS, etc.}
...
Each synthetic cohort contains $N = 100$ patients, and the experiment is repeated $M = 100$ times. Each fresh model is fitted using the MCMC-SAEM algorithm with $N_{\mathrm{iter}} = 3000$ iterations, and the subsequent personalisation step runs for $N_{\mathrm{perso}} = 500$ iterations. Naturally, the computational cost of this procedure is substantial, as it requires fitting $M$ separate joint models from scratch. Better results could be obtained by increasing $N$ and $M$, but this would further increase the computational burden. The chosen values of $N$ and $M$ represent a compromise between statistical power and practical feasibility, while still providing a robust assessment of the model's self-consistency.
\todo{changer avec les nouveaux résultats}

\subsection{Results and Interpretation}
\todo{changer avec les nouveaux résultats}
The results of the simulation study are summarised in Table~\ref{tab:population_recovery} for population parameters. The distributions of the Relative Estimation Error across the $M = 100$ repetitions are displayed in Figure~\ref{fig:ree_all}, while the quality of individual parameter recovery is examined in detail through the scatter plots of Figure~\ref{fig:param_comparison} and the Bland-Altman plots of Figure~\ref{fig:bland-altman}.

\paragraph{Population parameter recovery.}
The population reference time $t_0$ is among the best recovered parameters, with a relative bias of $0.072\%$ and a RRMSE of $2.082\%$. Its REE distribution in Figure~\ref{fig:ree_all} is centred almost exactly on zero with a tight interquartile range, indicating that it is well identified in this simulation setting. The observational noise standard deviation $\sigma$ is the most precisely estimated parameter in the study, with an RSE of $0.003$, a relative bias of $-3.897\%$, and a RRMSE of $3.910\%$. The near-equality of these last two quantities indicates that almost all the estimation error is attributable to a stable, slightly negative systematic offset rather than to variability across repetitions, a pattern visible in Figure~\ref{fig:ree_all} as a tight, uniformly sub-zero distribution. The time-shift standard deviation $\sigma_\tau$ and the trajectory parameters $\tilde{v}_0$ and $\tilde{g}$ are also well recovered, with relative biases in the range of $3.6\%$ to $7.4\%$ and RRMSEs below $11\%$; their REE distributions remain within moderate bounds across almost all repetitions, indicating that the main longitudinal sub-model parameters are reliably estimated under the dense visit schedules of this study.

The log-acceleration standard deviation $\sigma_\xi$ is overestimated by $27\%$ on average, with a RRMSE of $29.8\%$ and an RSE of $0.10$. One possible explanation is that, at this sample size, part of the unexplained individual variability is absorbed into the between-subject variance, inflating the recovered population standard deviation. The Weibull survival parameters $\tilde{\nu}$ and $\tilde{\rho}$ are both underestimated, with relative biases of $-18.1\%$ and $-28.2\%$ respectively. Several plausible factors may contribute to the difficulty of identifying these parameters. First, survival data are intrinsically sparse: with $N = 100$ patients, a substantial fraction of whom are right-censored, the effective information available to the survival sub-model is far smaller than that available to the longitudinal one. Second, the two Weibull parameters may be partially compensatory in the likelihood, so that changes in scale can be offset to some extent by changes in shape, producing unstable joint estimates. Third, the shape parameter $\rho$ is generally harder to identify, as it controls the functional form of the hazard acceleration and requires observing a sufficiently broad spread of event times; the RSE of $\tilde{\rho}$ (0.443) being about four times that of $\tilde{\nu}$ (0.113) is consistent with this additional difficulty.

Finally, the space-shift mixing coefficients $\beta$ and the source-to-survival coupling weights $\zeta$ present the largest relative errors, with RRMSEs of $212\%$ and $117\%$ respectively. These figures, however, must be interpreted with caution. The true mean of $\beta$ is $0.00858$, a value so close to zero that even a small absolute estimation error yields a very large relative one. The metric therefore overstates the practical difficulty of recovering this parameter. More fundamentally, both $\beta$ and $\zeta$ encode second-order effects that are difficult to separate from the dominant signal at moderate sample sizes: the space-shift coefficients require the simultaneous observation of consistent inter-biomarker ordering differences across many patients, while $\zeta$ requires resolving a systematic link between the latent source patterns $\mathbf{s}_i$ and the individual event hazard beyond the shared disease pace $e^{\xi_i}$. For $\zeta$, the combination of a very large relative bias and a very high RSE suggests substantial instability in this experimental regime. Overall, these results are consistent with known identifiability difficulties in high-dimensional joint models and would be expected to improve with larger sample sizes, although this was not evaluated here.
...
\hline
$\beta$ (mean)            & 0.00858  & 0.00107  & -87.535 & 212.287 & 15.5936 \\
$\tilde{g}$ (mean)        & 0.49849  & 0.46148  & -7.424  & 10.373  & 0.0786 \\
$\tilde{\rho}$            & 0.67007  & 0.48141  & -28.155 & 42.351  & 0.4426 \\
$\tilde{v}_0$ (mean)      & -4.15871 & -4.44446 & -6.871  & 8.690   & 0.0500 \\
$\tilde{\nu}$             & -1.91844 & -2.26636 & -18.135 & 22.449  & 0.1126 \\
$\sigma$                  & 0.39104  & 0.37580  & -3.897  & 3.910   & 0.0033 \\
$t_0$                     & 66.07919 & 66.12698 & 0.072   & 2.082   & 0.0209 \\
$\sigma_{\tau}$           & 10.36347 & 9.99169  & -3.587  & 7.826   & 0.0725 \\
$\sigma_{\xi}$            & 1.34546  & 1.70900  & 27.020  & 29.809  & 0.0996 \\
$\zeta$ (mean)            & -0.23890 & -0.01580 & 93.387  & 116.725 & 10.6431 \\
\hline
\end{tabular}
\end{table}
...
\paragraph{Individual random effect recovery.}
Figures~\ref{fig:param_comparison} and~\ref{fig:bland-altman} together provide a detailed assessment of individual parameter recovery by the personalisation algorithm. The ICC(3,1) values, averaged across the $M = 100$ repetitions, are $0.894 \pm 0.026$ for $\xi$ and $0.903 \pm 0.050$ for $\tau$. Both values indicate strong absolute agreement between the personalised and simulated parameters, well above the conventional threshold of $0.75$ for good agreement. The slightly higher standard deviation of the ICC for $\tau$ across repetitions (0.050 versus 0.026 for $\xi$) suggests that the recovery of the time shift is somewhat more sensitive to the particular realisation of the synthetic cohort. In both cases, the Pearson correlation modestly exceeds the ICC ($0.912 \pm 0.021$ versus $0.894 \pm 0.026$ for $\xi$; $0.916 \pm 0.038$ versus $0.903 \pm 0.050$ for $\tau$), a systematic gap that is consistent across repetitions and indicates a mild distortion between true and estimated values that the ICC penalises but the Pearson correlation ignores.

This distortion is made explicit in Figure~\ref{fig:param_comparison}. For $\xi$, the linear regression line is slightly steeper than the identity line, indicating that the personalisation algorithm marginally amplifies the true spread: patients with strongly positive or negative log-accelerations tend to receive estimates that are somewhat more extreme than their true values. This pattern is consistent with the upward bias in $\hat{\sigma}_{\xi}$ documented above: if the fitted prior variance is overestimated, the posterior regularisation is weaker than it should be, producing less shrinkage toward zero and a correspondingly expanded distribution of estimated log-accelerations. For $\tau$, the regression line is shallower than the identity, revealing the classic shrinkage pattern toward the prior mean: patients with a very early true disease onset tend to have their onset underestimated, while patients with a very late onset tend to have it overestimated. This is the expected consequence of Bayesian estimation under a Gaussian prior centred near the population mean; when the individual longitudinal data are not sufficiently informative to dominate the prior, the posterior is pulled back toward that central value.
...
The Bland-Altman plots in Figure~\ref{fig:bland-altman} confirm and refine these observations. For $\xi$, the bias is $-0.007$, negligible in absolute terms, and the limits of agreement are $[-1.350, +1.336]$. For $\tau$, the bias is $+0.061$ years, again negligible relative to the scale of $\sigma_\tau \approx 10$ years, and the limits of agreement span approximately $[-8.4, +8.5]$ years. For both parameters, the mean bias is therefore close to zero, indicating little overall offset at the population level. At the same time, the structure of the point clouds shows that this absence of global bias does not imply perfect agreement at all values. In particular, the Bland-Altman plot for $\tau$ displays a marked mean-dependent pattern consistent with the shrinkage already visible in Figure~\ref{fig:param_comparison}: extreme time shifts tend to be pulled back toward the centre of the distribution. The near-symmetry of the limits of agreement around zero confirms that the personalisation procedure does not introduce a large systematic offset on average, while the residual structure of the plots reveals conditional bias, especially for $\tau$.
